...
class YahooWeather extends Helper implements ISiteHelper
{
    ...
    public function buildQuery($city)
    {
        ...
        $this->url .= '?q=' . urlencode("select item.forecast from weather.forecast where woeid = {$woeid} and u='c' limit {$this->days}");
        $this->url .= '&format=json';
        $this->url .= '&env=store://datatables.org/alltableswithkeys';
    }
    ...
    public function addWeatherData()
    {
        ...
        $q = $data['query'];
        $results = $q['results']['channel'];

        $count = count($results);


        for ($i = 0; $i < $count; $i++) {
            $period = $results[$i]['item']['forecast'];
